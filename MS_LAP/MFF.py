...
class MFF:
    ...
    '''
    def __init__(self, input, map_mode='lap', window_size=None, 
        ...
        if window_size is None:
            window_size = max(5e-5*self.H*self.W,3)
        self.window_size=int(2*(window_size//2)+1)
    ...
    def mff_ssim(self, fused_image):
        ...
        for i in range(self.input.shape[0]):
            ssim_map.append(ssim(self.input[i,:,:,:][None,:,:,:], fused_image,
                                 self.window_size))
        ssim_map = torch.cat(ssim_map, dim=0)
    ...
    def mapped_ssim(self, a, map):
        ...
        if len(map.shape)==3:
            ...
            output = 0.
            map = map.permute(2,0,1)[:,None,:,:].repeat(1,self.C,1,1)
    
            output = torch.sum(map*a, dim=0)
        elif len(map.shape)==2:
            map = map.to(a.dtype)
            output = a[0,:,:]*map+a[1,:,:]*(1-map)
        return output

    def get_map(self, map_mode):
        '''
        Estimate the focus map.
        '''
        if type(map_mode) is str:
            ...
            elif map_mode in ['lap', 'laplacian']:
                return map_generator.lap_map(self.input, self.window_size)
            elif map_mode in ['gfdf']:
                return map_generator.gfdf_map(self.input)
            
        elif type(map_mode) is torch.Tensor:
    ...
    def save_image(self, root):
        ...
        '''
        root0 = os.path.join(*root.split('/')[:-1])
